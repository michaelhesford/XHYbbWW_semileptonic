...
if __name__ == '__main__':
    ...
    cmd = 'eos {} ls {}pileup/'.format(redirector,eos_path)
    
    rawFiles = subprocess.check_output(cmd, shell=True).decode()
    files = rawFiles.split('\n')
    for fName in files:
        ExecuteCmd('xrdcp {}{}pileup/{} /uscms/home/mhesford/nobackup/XHYbbWW/CMSSW_12_3_0/src/semileptonic/pileup/'.format(redirector, eos_path, fName))
    '''
    ...
    for ttbar_file in subprocess.check_output(cmd+'pileup_ttbar-allhad17/*', shell=True).split('\n'):
        ExecuteCmd('xrdcp {}{}pileup_ttbar-allhad17/{} /uscms/home/mhesford/nobackup/XHYbbWW/CMSSW_12_3_0/src/semileptonic/pileup/'.format(redirector,eos_path,ttbar_file))
