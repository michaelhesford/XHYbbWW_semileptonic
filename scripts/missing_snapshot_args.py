'''
Checks for missing snapshots and returns an args file containing jobs which need to be re-run
'''
...
for g in groups:
    if not 'Data' in g:
        for year in ['16APV','16','17','18']:
            ...
            #eos_path = '/store/user/mhesford/XHYbbWW_semileptonic/PNetSnapshots/PNetSnapshot_{}_{}_*'.format(g,year)
            ...
            nfiles_raw = len(open(f'raw_nano/{g}_{year}.txt').readlines()) #number of raw NanoAOD files
            if len(files) != 1: # expect nfiles = njobs = 1 for signal samples
                files = [f for f in files if str(nfiles_raw)+'.root' not in f] # remove any files resulting from later job splitting
        ...
        #eos_path = '/store/user/mhesford/XHYbbWW_semileptonic/PNetSnapshots/PNetSnapshot_{}_{}_*'.format(setname,year)
        cmd = 'eos {} ls {}'.format(redirector,eos_path)
        rawFiles = subprocess.check_output(cmd, shell=True).decode()
        files = rawFiles.split('\n')[:-1]
        nfiles_raw = len(open(f'raw_nano/{setname}_{year}.txt').readlines()) #number of raw NanoAOD files
        files = [f for f in files if str(nfiles_raw)+'.root' not in f] # remove any files resulting from later job splitting
        if len(files) == 0:
