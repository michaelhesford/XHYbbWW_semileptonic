...
if __name__ == "__main__":
    ...
    DeltaPhi_objs['TagJets'] = ['Dijet_phi','Dijet_phi']
    ...
    ana.a.Define('MET_vect','hardware::TLvector(MET_pt,0,MET_phi,0)') #neutrino mass negligable - for now, assuming p_missing has no z component 

    # Transverse/invariant masses of W -> l/v (should come out around 80 GeV)
    ana.a.Define('W_massTran','hardware::TransverseMass(MET_pt,Lepton_pt,MET_phi,Lepton_phi)') #note that I'm using the pre-existing root function for this
    ana.a.Define('W_massTran_genMET','hardware::TransverseMass(MET_fiducialGenPt,Lepton_pt,MET_fiducialGenPhi,Lepton_phi)') #using generator-level MET variables
    ana.a.Define('LeptonMET_massInv','hardware::InvariantMass({Lepton_vect,MET_vect})')
    ana.a.Define('LeptonMET_massInv_genMET','hardware::InvariantMass({Lepton_vect,{MET_fiducialGenPt,0,MET_fiducialGenPhi,0}})') #using generator-level quantities
